...
The runner calls the pinned npm CLI `-s=<unique-session> open <url> --config scripts/browser-bench/cli.config.json`, `-s=<session> run-code --filename=/tmp/kalada-99-run-<random>/run-code.js`, `-s=<session> requests --static`, `-s=<session> close`. Exact CLI open/requests/close output is preserved per batch in the raw JSON; run-code stdout is losslessly gzip+base64 encoded in `runLogGzipBase64` (decode with `Buffer.from(value, 'base64')` and `gunzipSync`). The uniquely owned temporary script/directory is removed in `finally`, including when CLI fails; the close is best-effort, and close failure is recorded as a batch failure. Each batch is a fresh non-persistent browser session; fixture navigations inside it are warm-cache. Default and minimum calibration is **21 total sessions** (one warmup plus 20 measured); `--batches=20` fails before launch. `--smoke=true` is the explicit one-batch exception, constrained to output under `/tmp/opencode/`, never calibration. No forced GC, no throttling, 230 ms settling before/after edit; the latter includes the 150 ms runtime debounce. Run-code executes the five edits in the page (CLI IPC is not timed). Actual `page.keyboard.press('Space')` and rapid two-revision supersession are independent correctness checks, not timing observations. The default loopback origin is enforced and external requests aborted; network request URLs are saved in raw.
...
**Browser provenance check added after original A cohorts:** The remediation confirmation below additionally records `page.context().browser()?.version()` and CDP `Browser.getVersion` per batch, outside the timed edit, and rejects mismatches with the expected 154.0.8037.0. These independent in-browser observations corroborate version and protocol revision, **not executable path identity**; the executable path in the manifest is an inspected managed installation, not proof of which binary the CLI launched. The CLI trace context also reports its own bundled Playwright `1.64.0-alpha-1789764292000`, separate from repository Playwright 1.63.

**Separate diagnostic CLI trace (never a timed calibration session):** [`99-trace-summary.json`](99-trace-summary.json) publishes event counts, browser/CLI provenance, local ignored trace path and SHA-256 `25609aa4a341081e6fc106608372a71f36dec9b054ae93fa1a486195ea09ea3b`. On this machine the full synthetic-only trace remains at `.playwright-cli/traces/trace-1790156711989.trace` (131,259 bytes) with empty `.network` log; trace snapshots/screencasts are deliberately not published, since a trace can contain editor content. To regenerate in a disposable named session outside collection, run:

```sh
CLI=scripts/browser-bench/node_modules/.bin/playwright-cli
$CLI -s=k99-trace-20260923 open http://127.0.0.1:4179/kalada/ --config scripts/browser-bench/cli.config.json
$CLI -s=k99-trace-20260923 tracing-start
$CLI -s=k99-trace-20260923 run-code 'async (page) => { const version=page.context().browser()?.version(); const cdp=await page.context().newCDPSession(page); const product=await cdp.send("Browser.getVersion"); await cdp.detach(); await page.locator(".cm-content").click(); await page.keyboard.press("End"); await page.keyboard.press("Space"); console.log(JSON.stringify({version,product,marked:await page.locator(".cm-content").textContent()})); }'
$CLI -s=k99-trace-20260923 requests --static
$CLI -s=k99-trace-20260923 tracing-stop
$CLI -s=k99-trace-20260923 close
node scripts/browser-bench/trace-summary.mjs .playwright-cli/traces/trace-<printed-id>.trace .playwright-cli/traces/trace-<printed-id>.network /tmp/opencode/99-trace-summary.json
```

The trace has CLI action/snapshot evidence, **not** parse-stage timings, paint timestamps or total CPU. The trace session ran before the remediated A confirmation and did not contribute to any A raw statistics. The deprecated runner's 42 leftover scripts identified by published A session IDs were removed only after owner/contents checks with `cleanup-owned.mjs`; the Auditor's separate smoke file and the pre-existing `bench99probe` session were not touched. The 98-line CLI transport wrapper is an explicit code-principles exception submitted for audit: `run-code --filename` must provide one self-contained serializable function, while its measured in-page callback remains under 50 lines; splitting into extra CLI calls would change the one-call-per-batch timing procedure. No production file has this exception.
...
**Remediation A confirmation:** [`99-a-remediation-raw.json`](99-a-remediation-raw.json) and [`99-a-remediation-summary.json`](99-a-remediation-summary.json) record the post-audit version/CDP check and finally cleanup on the same immutable A artifact. 21 fresh sessions, 1 warmup + 20 valid, zero failures. Median/p95 dispatch ms: typical 7.9/8.7; many-tokens 17.7/19.6; padded-valid 8.4/8.8; recovery 17.3/18.1; CRLF/astral 7.0/8.1. Median/p95 rAF proxy ms: 11.5/18.2, 22.5/24.2, 16.9/23.3, 20.6/21.8, 10.5/14.5, respectively. IQR and spread are in the summary. The original two cohorts are preserved unchanged; this third cohort does not retroactively change their statistics. CLI smoke with `--smoke=true` wrote only `/tmp/opencode/kalada-99-remediation-smoke.json` and passed. An attempted `--batches=20` was rejected before browser launch as intended. No runner-owned temporary run-code directories remain after collection.
