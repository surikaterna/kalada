...
import type { DiffEntry, PackageManifest, ReleaseException } from "./types.js";

const recordPath = /^release-exceptions\/(?!schema\.v1\.json$)[^/]+\.json$/u;
...
export function validateException(
  ...
  const authorized = entries.filter((entry) => entry.path !== path);
...
function assertIdentity(record: ReleaseException, base: string, repository: string): void {
...
function validateEvidence(record: ReleaseException): void {
